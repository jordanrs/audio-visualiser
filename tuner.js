...
(function() {
  (function() {
    var bars, frequencyData, gradient, loadCanvas, loadFileIntoAudio, localFileLoad, processStream, setupCSSVisualisation, soundAnalyser, timeData, updateGraph;
    window.audioMixer = {};
    bars = null;
    soundAnalyser = [];
    ...
    localFileLoad = function(file, audioCtx, audioBufferNode) {
      var audioFileReader;
      audioFileReader = new FileReader();
      audioFileReader.onload = function(event) {
        return audioCtx.decodeAudioData(event.target.result, function(buffer) {
          return audioBufferNode.buffer = buffer;
        });
      };
      return audioFileReader.readAsArrayBuffer(file.files[0]);
    };
    loadFileIntoAudio = function(file, audioEl) {
      return audioEl.src = URL.createObjectURL(file.files[0]);
    };
    setupCSSVisualisation = function() {
      var barGraph, barSpacing, i, _i, _ref;
      barGraph = $("#bar-graph");
      barSpacing = 100 / audioMixer.analyser.frequencyBinCount;
      for (i = _i = 0, _ref = audioMixer.analyser.frequencyBinCount; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        $("<div/>").css({
          "left": "" + (i * barSpacing) + "%",
          "width": "" + barSpacing + "%"
        }).appendTo(barGraph);
      }
      return bars = $("#bar-graph > div");
    };
    updateGraph = function() {
      var bar, data, i, imageData, point, _i, _j, _k, _len, _len1, _len2;
      ...
      for (i = _i = 0, _len = frequencyData.length; _i < _len; i = ++_i) {
        point = frequencyData[i];
      ...
      for (i = _k = 0, _len2 = timeData.length; _k < _len2; i = ++_k) {
        data = timeData[i];
        audioMixer.canvasContext.lineTo(i * 3, data);
      }
      audioMixer.canvasContext.strokeStyle = "green";
      return audioMixer.canvasContext.stroke();
    };
    loadCanvas = function() {
      var canvas, spectrum;
      ...
      if (canvas.getContext) {
        ...
        return audioMixer.spectrum = spectrum.getContext('2d');
      } else {
    ...
    processStream = function(stream) {
      ...
      return audioMixer.mic.connect(audioMixer.analyser);
    };
